...
import { PLANNER_ACTIVITY_LEVELS, CARB_PERCENTAGES, CUISINE_TYPES, DIETARY_PREFERENCES } from '../constants';
...
const PersonalizedPlanner: React.FC = () => {
    ...
    const handleCalculateAndPlan = async () => {
        ...
        try {
            ...
            setError(e.message || "เกิดข้อผิดพลาดในการสร้างแผนอาหาร");
        } finally {
            setLoading(false);
        }
    };
    ...
    const FormView = () => (
        <div className="max-w-lg mx-auto bg-white dark:bg-gray-800 p-6 md:p-8 rounded-2xl shadow-lg space-y-4 animate-fade-in">
            <h2 className="text-2xl font-bold text-gray-800 dark:text-white mb-2 text-center">นักวางแผนโภชนาการ</h2>
            <p className="text-center text-gray-600 dark:text-gray-300 -mt-2 mb-4">
                กรอกข้อมูลเพื่อคำนวณเป้าหมายและสร้างแผนอาหาร AI
            </p>
            ...
            <div className="border-t dark:border-gray-700 pt-4 space-y-4">
                <h3 className="text-lg font-bold text-gray-800 dark:text-white text-center">ประเภทอาหาร</h3>
                 <div>
                 ...
                 <div>
                    <label className="text-sm font-medium text-gray-700 dark:text-gray-300">ข้อจำกัด</label>
    ...
    const ResultsView = () => (
        <div className="animate-fade-in space-y-8">
            <div className="flex justify-between items-center print:hidden">
                <h2 className="text-2xl font-bold text-gray-800 dark:text-white">ผลลัพธ์และแผนอาหาร</h2>
                <button
            ...
            {loading && !results && (
                 <div className="bg-white dark:bg-gray-800 p-8 rounded-2xl shadow-lg text-center">
                    <div className="flex flex-col items-center justify-center gap-4">
                        ...
                        <p className="text-teal-600 dark:text-teal-400 font-medium">กำลังคำนวณข้อมูลสุขภาพ...</p>
                    </div>
                 </div>
            )}
            ...
            {results && (
                <div id="results-summary" className="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-lg animate-fade-in print:hidden">
                    <h2 className="text-2xl font-bold text-gray-800 dark:text-white mb-4 text-center">สรุปข้อมูลสุขภาพของคุณ</h2>
            ...
            <div id="meal-plan-section" className="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-lg animate-fade-in print:shadow-none dark:print:bg-white dark:print:text-black">
                 {loading && results && (
                     <div className="text-center">
                        <div className="flex flex-col items-center justify-center gap-4">
                            <div className="w-12 h-12 border-4 border-t-teal-500 border-gray-200 dark:border-gray-600 rounded-full animate-spin"></div>
                            <p className="text-teal-600 dark:text-teal-400 font-medium">AI กำลังสร้างแผนอาหาร... โปรดรอสักครู่</p>
                        </div>
                     </div>
                )}
                ...
                {mealPlan && results && (
                    <div className="printable-area">
                         <div className="flex justify-between items-center mb-4 print:hidden">
                            <h2 className="text-2xl font-bold text-gray-800 dark:text-white">แผนอาหาร 7 วันของคุณ</h2>
                         ...
                         <p className="mb-4 text-gray-600 dark:text-gray-300 print:hidden">
                            แผนอาหารนี้ถูกสร้างโดย AI เพื่อให้ใกล้เคียงกับเป้าหมาย TDEE (~{results.tdee.toFixed(0)} kcal) และเป้าหมายสารอาหารหลักของคุณ โดยเน้นอาหาร <span className="font-semibold">{formData.cuisine}</span> และเป็นแบบ <span className="font-semibold">{formData.diet}</span>
                        </p>
                        <div className="overflow-x-auto">
                            <table className="w-full border-collapse text-sm dark:text-gray-200 dark:print:text-black">
                                <thead className="bg-gray-100 dark:bg-gray-700">
                                    <tr>
                                        <th className="p-3 font-semibold text-left border-b dark:border-gray-600">วัน/มื้อ</th>
                                        <th className="p-3 font-semibold text-left border-b dark:border-gray-600">เมนู</th>
                                        <th className="p-3 font-semibold text-right border-b dark:border-gray-600">โปรตีน (g)</th>
                                        <th className="p-3 font-semibold text-right border-b dark:border-gray-600">คาร์บ (g)</th>
                                        <th className="p-3 font-semibold text-right border-b dark:border-gray-600">ไขมัน (g)</th>
                                        <th className="p-3 font-semibold text-right border-b dark:border-gray-600">แคลอรี่</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {mealPlan.map(day => (
                                        <React.Fragment key={day.day}>
                                            <tr className="bg-gray-50 dark:bg-gray-700/50 font-bold"><td colSpan={6} className="p-2 text-gray-700 dark:text-gray-100">{day.day}</td></tr>
                                            <tr><td className="p-2 border-b dark:border-gray-600 text-gray-500 dark:text-gray-400">เช้า</td><td className="p-2 border-b dark:border-gray-600">{day.breakfast.menu}</td><td className="p-2 border-b dark:border-gray-600 text-right">{day.breakfast.protein.toFixed(0)}</td><td className="p-2 border-b dark:border-gray-600 text-right">{day.breakfast.carbohydrate.toFixed(0)}</td><td className="p-2 border-b dark:border-gray-600 text-right">{day.breakfast.fat.toFixed(0)}</td><td className="p-2 border-b dark:border-gray-600 text-right">{day.breakfast.calories.toFixed(0)}</td></tr>
                                            <tr><td className="p-2 border-b dark:border-gray-600 text-gray-500 dark:text-gray-400">กลางวัน</td><td className="p-2 border-b dark:border-gray-600">{day.lunch.menu}</td><td className="p-2 border-b dark:border-gray-600 text-right">{day.lunch.protein.toFixed(0)}</td><td className="p-2 border-b dark:border-gray-600 text-right">{day.lunch.carbohydrate.toFixed(0)}</td><td className="p-2 border-b dark:border-gray-600 text-right">{day.lunch.fat.toFixed(0)}</td><td className="p-2 border-b dark:border-gray-600 text-right">{day.lunch.calories.toFixed(0)}</td></tr>
                                            <tr><td className="p-2 border-b dark:border-gray-600 text-gray-500 dark:text-gray-400">เย็น</td><td className="p-2 border-b dark:border-gray-600">{day.dinner.menu}</td><td className="p-2 border-b dark:border-gray-600 text-right">{day.dinner.protein.toFixed(0)}</td><td className="p-2 border-b dark:border-gray-600 text-right">{day.dinner.carbohydrate.toFixed(0)}</td><td className="p-2 border-b dark:border-gray-600 text-right">{day.dinner.fat.toFixed(0)}</td><td className="p-2 border-b dark:border-gray-600 text-right">{day.dinner.calories.toFixed(0)}</td></tr>
                                            <tr className="bg-gray-100 dark:bg-gray-700 font-semibold">
                                                <td colSpan={2} className="p-2 text-right">รวมต่อวัน</td>
                                                <td className="p-2 text-right">{day.dailyTotal.protein.toFixed(0)}</td>
                                                <td className="p-2 text-right">{day.dailyTotal.carbohydrate.toFixed(0)}</td>
                                                <td className="p-2 text-right">{day.dailyTotal.fat.toFixed(0)}</td>
                                                <td className="p-2 text-right">{day.dailyTotal.calories.toFixed(0)}</td>
                                            </tr>
                                        </React.Fragment>
                                    ))}
                                </tbody>
